...
class StateStringUtils:
    ...
    def add_drop_out_states(self, state_list):
        ...
        '''
        new_state_list = []
        list_withoutcomplex=[state for state in state_list if state.type=='simple']
        number_fc = len([state for state in list_withoutcomplex if state.layer_type == 'fc'])
        number_gap = len([state for state in list_withoutcomplex if state.layer_type == 'gap'])
        number_drop_layers = (len(list_withoutcomplex) - number_gap - number_fc)/2 + number_fc
        drop_number = 1
        for i in range(len(state_list)):
            new_state_list.append(state_list[i])
            if state_list[i].type=='simple':
    ...
    def state_list_to_string(self, state_list):
        ...
        while i < len(state_list):
            ...
            if state.type=='simple':
                ...
                if len(state.liststate_branch0)>0:
                    for j in range(len(state.liststate_branch0)):
                        strings.append(self.state_to_string(state.liststate_branch0[j]))
                if len(state.liststate_branch1) > 0:
                    for k in range(len(state.liststate_branch1)):
                        strings.append(self.state_to_string(state.liststate_branch1[k]))
                if len(state.liststate_branch2) > 0:
                    for l in range(len(state.liststate_branch2)):
                        strings.append(self.state_to_string(state.liststate_branch2[l]))
    ...
    def state_to_string(self, state):
        ...
        if state.terminate == 1:
            ...
            return 'C(%i,%i,%i,%i)' % (state.filter_depth, state.filter_size, state.stride,state.branch)
        elif state.layer_type == 'gap':
            return 'GAP(%i,%i)' % (self.output_number)
        elif state.layer_type == 'pool':
            return 'P(%i,%i,%i)' % (state.filter_size, state.stride,state.branch)
        elif state.layer_type == 'fc':
            return 'FC(%i,%i)' % (state.fc_size)
        elif state.layer_type == 'dropout':
            return 'D(%i,%i,%i)' % (state.filter_depth, state.fc_size,state.branch) ##SUPER BAD i am using fc_size and filter depth -- should fix later
        elif state.layer_type == 'concat':
            return 'Con(%i,%i)' % (state.image_size,
                                    state.branch)  ##SUPER BAD i am using fc_size and filter depth -- should fix later
    ...
    def convert_model_string_to_states(self, parsed_list, start_state=None):
        ...
        for layer in parsed_list:
            if layer[0] == 'conv':
                ...
                states.append(se.State(layer_type='concat',
                                       layer_depth=states[-1].layer_depth,
                                       filter_depth=layer[1],
                                       filter_size=0,
                                       stride=0,
                                       branch=layer[4],
                                       image_size=self.enum._calc_new_image_concat(self, states[-1]),
                                       fc_size=0,
                                       terminate=0))
        ...
        for j in range(len(states)):
            ...
            if states[j].branch!=-1:
                ...
                list_new.append(se.Statecomplex(list0,list1,lis2,terminate==0))
                j=i+1
