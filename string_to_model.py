...
class Parser:
    ...
    def unpack_list(self, net_list, data, label):
        ...
        for layer_number in range(len(net_list)):
            #case simple and case complex
            if net_list[layer_number].type=='complex':
                layer_spec = net_list[layer_number]
                bottomcomplex = self.unpack_complexitem(layer_spec,
                                          net_list[layer_number - 1].image_size if layer_number else self.hp.IMAGE_HEIGHT,
                                          layer_number,
                                          bottom,
                                          label)
            if net_list[layer_number].type=='simple' and net_list[layer_number-1].type=='simple':
                layer_spec = net_list[layer_number]
                bottom = self.unpack_item(layer_spec,
                                          ...
                                          bottom,
                                          label)
            if net_list[layer_number].type == 'simple' and net_list[layer_number - 1].type == 'complex':
                layer_spec = net_list[layer_number]
                bottom = self.unpack_itemfromC(layer_spec,
                                          net_list[
                                              layer_number - 1].image_size if layer_number else self.hp.IMAGE_HEIGHT,
                                          layer_number,
                                          bottomcomplex,
                                          label)
    ...
    def unpack_complexitem(self,layer, previous_image_size, layer_number, bottom, label=None):
        previousdata_list=[]
        if layer.liststate_branch0!=None:
            bottom0=self.unpack_item(self, layer.liststate_branch0[0], previous_image_size, layer_number, bottom, label=None)
            for i in range(1,len(layer.liststate_branch0)):
                bottom0 =self.unpack_item(self, layer.liststate_branch0[i], layer.liststate_branch0[i-1].image_size, layer_number, bottom0, label=None)
            previousdata_list.append(bottom0)
        if layer.liststate_branch1 != None:
            bottom1=self.unpack_item(self, layer.liststate_branch1[0], previous_image_size, layer_number, bottom,
                             label=None)
            for j in range(1, len(layer.liststate_branch1)):
                bottom1=self.unpack_item(self, layer.liststate_branch1[j], layer.liststate_branch1[j - 1].image_size,
                                 layer_number, bottom1, label=None)
            previousdata_list.append(bottom1)
        if layer.liststate_branch2 != None:
            bottom2 =self.unpack_item(self, layer.liststate_branch2[0], previous_image_size, layer_number, bottom,
                             label=None)
            for h in range(1, len(layer.liststate_branch2)):
                bottom2 =self.unpack_item(self, layer.liststate_branch2[h], layer.liststate_branch2[h - 1].image_size,
                                 layer_number, bottom2, label=None)
            previousdata_list.append(bottom2)
        return previousdata_list

    def unpack_itemfromC(self, layer, previous_image_size, layer_number, bottomcomplex, label=None):

        #case concat layer:
        if len(bottomcomplex)>2:
            bottom = cl.Concat(bottomcomplex[0],bottomcomplex[1],bottomcomplex[2], axis=1)
        if len(bottomcomplex)==2:
            bottom = cl.Concat(bottomcomplex[0], bottomcomplex[1], concat_param={'axis':1})
        return bottom
    ...
    def unpack_item(self, layer, previous_image_size, layer_number, bottom, label=None):
        ...
        if layer.layer_type == 'lstm':
            bottom=cl.LSTM(bottom,weight_filler=dict(type='xavier'),num_output=layer.seq_lengh)
            return bottom
